  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(
        "https://ib.jetafx.com/api/customer/auth/getCustomer",
      );
  ...
  const deleteCustomer = async (clientId: number) => {
    try {
      ...
      if (result.ResponseCode === 200) {
        ...
        setData((prev) =>
          prev.filter((customer) => customer.ClientId !== clientId),
        );
            ...
            <div className="flex items-center gap-2 p-1">
              ...
              <button
                onClick={async () => {
                  ...
                  const fullCustomer = await fetchCustomerById(
                    customer.ClientId,
                  );
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="bg-white/80 dark:bg-black/60 backdrop-blur-sm border border-gray-200/50 rounded-2xl p-8 shadow-xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 items-end">
          ...
          <Input
            ...
            onChange={(v: string) =>
              setFilters({ ...filters, searchClientId: v })
            }
          />
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white/50 backdrop-blur-sm border border-gray-200/50 rounded-xl p-6">
        <span className="text-sm text-gray-600 font-medium">
          Showing {table.getRowModel().rows.length} of {data.length} customer
          records
        </span>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-gray-600 text-sm leading-relaxed">
                  ...
                  <strong className="font-mono text-red-600">
                    #{customerToDelete.ClientId}
                  </strong>{" "}
                  - <strong>{customerToDelete.ClientName}</strong>.
                </p>
              </div>
            </div>
